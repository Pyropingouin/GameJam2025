...
func _ready() -> void:
	...
	hide_all_animations_assets()

	# Set the Mana Counter
...
func attack_enemies():
	if ennemyNextMove.type == "Attack":
		if squirrel_enemy.squirrel_type == "Normal":
			...
			elif squirrel_enemy.squirrel_name == "Fleur":
				animations.get_node("FleurEnemy").visible = true
				animations.get_node("AnimationPlayer").play("flower_hit")
...
func enter_combat_mode():
	#print("HIHI")
	#print(get_tree().change_scene_to_file(MAIN_SCENE_PATH))
...
#func start_game():
	# Seulement afficher MainMenuScene
	#all_invisible()
...
func hide_all_animations_assets():
